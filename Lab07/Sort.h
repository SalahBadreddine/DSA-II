...
{
    for( int p = 1; p < a.size( ); ++p ) // O(n)
    {
        Comparable tmp = std::move( a[ p ] ); // we store the value of the current index

        int j;
        // we loop to the left side, until we an element that is less than a[p] or we arrive to index 0 
        for( j = p; j > 0 && tmp < a[ j - 1 ]; --j ) 
            a[ j ] = std::move( a[ j - 1 ] ); // we shift to the right 
        a[ j ] = std::move( tmp ); // we insert the element in its correct position 
    }
}
